...
class SequencePosition(form.Form):
    ...
    def position(self):
        ...
        if hasattr(self.sequence, "free_symbols") and sympy.symbols("n") in self.sequence.free_symbols:
            general_term = self.sequence
        # solve for the general term
        else:
            general_term = sequence_term.SequenceTerm(sequence=str(self.sequence)).term.args[1]
